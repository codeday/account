...
export default function Home({ user, token, logIn, ...props }) {
  ...
  return (
    <Page slug="/" isLoggedIn={true}>
      ...
      <UserProperties token={token} user={merge(user, changes)} fields={["username", "picture", 'familyName', 'givenName', "displayNameFormat", "pronoun", (user.badges ? "badges" : null), "phoneNumber", "bio", (user.roles.find((role) => role.name === "Volunteer") ? "title" : null), "discord"]} onChange={setChanges} />
